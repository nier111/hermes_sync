...
### 委托前把目标路径和确切命令钉死在 context 里

子 agent 默认在它自己所在的仓库/目录里执行 —— 你说"帮我更新"，它会去更新它自己那套系统，
而真正要更新的对象可能是另一个部署。委托 context 必须写明：绝对路径、完整命令、以及哪些
东西不许动（"只读，不要 update 你自己"）。同理，被委托方回报"做过了/更新过了"只是自述：
更新类操作要自己复核目标 HEAD / 版本 / DB schema 再下结论，不要直接转述给用户。
