...
- **Two separate installs — do not confuse them (verified 2026-09-19)**: the source checkout at `~/projects/openclaw` is only what `pnpm openclaw ...` runs; the LIVE gateway (systemd `openclaw-gateway.service`) runs the **npm-global copy** at `~/.nvm/versions/node/v22.22.3/lib/node_modules/openclaw`. `git pull` + `pnpm build` in the checkout does NOT upgrade the running gateway. Upgrading the gateway = rebuild the package tarball and `npm install -g` it; the full verified procedure is in `references/global-upgrade-via-npm-pack.md` (`pnpm build:package` → `pnpm pack --skip-build --allow-unreleased-changelog` → backup → stop service → `npm install -g` with `--allow-scripts` → start service). `openclaw update` is the package-release path and has been failing here (pitfall 7 in the `openclaw` skill): don't make it the plan for "update OpenClaw".
